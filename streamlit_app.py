...
# Modo compacto — menos dados na tela, mais leve para iPad/Safari
COMPACTO = st.sidebar.toggle(
    "📱 Modo compacto (iPad)",
    value=st.session_state.get("modo_compacto", False),
    help="Reduz dados exibidos e fecha expanders automaticamente. "
         "Use no iPad ou conexões lentas.",
...
def _exibir_arbitrage(arbs: list[dict], banca_sidebar: float, key_prefix: str) -> None:
    ...
    for arb in arbs[:10]:
        ...
        with st.expander(f"📌 {arb['jogo']}  |  {arb['mercado']}  |  Lucro: **{arb['profit_pct']:.2f}%**", expanded=False if COMPACTO else False):
...
if COMPACTO:
    st.info("📱 **Modo compacto ativo** — exibindo até 5 picks e expanders fechados. "
            "Desative na sidebar para ver tudo.", icon="📱")
